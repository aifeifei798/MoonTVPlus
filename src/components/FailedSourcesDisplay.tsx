...
export default function FailedSourcesDisplay({
  ...
  return (
    <div className='relative' ref={containerRef}>
      <button
        className='flex items-center gap-2 hover:bg-amber-50 dark:hover:bg-amber-900/20 px-2 py-1 rounded-sm transition-colors'
          ...
          <div className='space-y-2 max-h-60 overflow-y-auto'>
            {failedSources.map((source, index) => (
              <div
                ...
                className='p-3 bg-amber-50 dark:bg-amber-900/20 rounded-sm border border-amber-100 dark:border-amber-800/30'
              >
                <div className='flex items-center gap-2 mb-1'>
                  <span className='font-medium text-amber-800 dark:text-amber-200 text-sm'>
                    {source.name}
                  </span>
                  <span className='text-xs text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-800/50 px-2 py-0.5 rounded-sm'>
                    {source.key}
                  </span>
                </div>
                <p
                  className={`text-xs break-words ${getErrorColor(source.error)}`}
                >
                  错误信息: {source.error}
                </p>
              </div>
